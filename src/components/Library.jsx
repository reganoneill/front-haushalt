...
export default class Library extends Component {
  ...
  backUpTrack(track) {
    console.log(
      `you just hit backUpTrack with this id which we'll use to upload this track to s3: ${track}`
    );
  ...
  renderBackupOrPlay(track) {
    ...
    if (track.backedUp) {
      ...
      return <span onClick={e => this.backUpTrack(track.id)}>backup</span>;
    }
  }
      ...
      <div className="libraryContainer">
        ...
        <div className="listInnerContainer">
          <table className="listTable table table-striped sticky-header">
            ...
            <tbody className="songsTableBody">{this.renderSongsTable()}</tbody>
          </table>
        </div>
      </div>
    );
